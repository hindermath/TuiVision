...
- [ ] CHK001 Are `plan.md`, `spec.md`, `research.md`, `data-model.md`, `quickstart.md`, and the acceptance contract all anchored to `015-didactic-comment-hardening`? [Completeness, Plan §Summary]
...
- [ ] CHK002 Is the binding input `Lastenheft_07_Didactic-Inline-Code-Comment-Hardening.md` preserved without expanding the accepted scope? [Traceability, Spec §FR-001]
...
- [ ] CHK003 Is the required ordering after `014-wave1-functional-hardening` and before Wave-1 visual remediation stated consistently? [Consistency, Plan §Baseline Assumptions]
...
- [ ] CHK004 Are all explicit out-of-scope boundaries repeated where task generation needs them? [Completeness, Plan §Constraints, Contract §1]
...
- [ ] CHK005 Is `specs/015-didactic-comment-hardening/pr-evidence.md` defined as the single primary evidence ledger? [Clarity, Plan §Summary, Contract §2]
...
- [ ] CHK006 Are the required evidence columns complete enough for every reviewed file or named flow area? [Completeness, Data Model §FeatureEvidenceEntry]
...
- [ ] CHK007 Are all hotspot categories from the contract represented in the data model with stable allowed values? [Coverage, Contract §3, Data Model §HotspotCategory]
...
- [ ] CHK008 Does the plan explain how categories with no current comment need will still be evidenced? [Coverage, Plan §Phase 2]
...
- [ ] CHK009 Are cross-file flows allowed as review areas when a hotspot is not owned by one file? [Clarity, Data Model §ReviewArea]
...
- [ ] CHK010 Is the five-value decision model exact and closed across all planning artifacts? [Consistency, Spec §FR-009, Research Decision 3]
...
- [ ] CHK011 Are `CommentAdequate` and `NoCommentNeeded` distinguishable for unchanged code? [Clarity, Contract §4]
...
- [ ] CHK012 Are `CommentNeeded` and `UpdateExistingComment` distinguishable for task decomposition? [Clarity, Data Model §CommentDecision]
...
- [ ] CHK013 Is `FollowUpHardening` bounded so it cannot become hidden runtime work inside 015? [Scope Control, Spec §FR-013]
...
- [ ] CHK014 Are the didactic comment purposes limited to why, trade-off, constraint, historical deviation, or proof boundary? [Clarity, Spec §FR-014]
...
- [ ] CHK015 Is the normal 1-to-3-line intensity rule present with explicit longer-comment exceptions? [Measurability, Spec §FR-016]
...
- [ ] CHK016 Is German-first/English-second CEFR-B2 scoped to didactic explanation blocks without touching license or tool markers? [Consistency, Spec §FR-017, §FR-018]
...
- [ ] CHK017 Are trivial comments explicitly rejected with objective examples? [Clarity, Spec §FR-015]
...
- [ ] CHK018 Are smoke-test helpers treated as first-class review targets? [Coverage, Research Decision 4]
...
- [ ] CHK019 Are proof purpose, stability reason, and proof boundary required for non-obvious helper paths? [Completeness, Data Model §SmokeProofBoundary]
...
- [ ] CHK020 Are setup-only and supplemental helpers protected from being overstated as primary behavior proof? [Acceptance, Contract §6]
...
- [ ] CHK021 Are rendering snapshot, buffer/cell, and terminal fallback boundaries covered without forcing runtime changes? [Scope Control, Plan §Phase 2]
...
- [ ] CHK022 Is `tv203s/` consistently read-only and comprehension-focused? [Consistency, Plan §Constraints, Contract §7]
...
- [ ] CHK023 Are historical deviations reviewed only when they clarify modern code or proof boundaries? [Scope Control, Research Decision 7]
...
- [ ] CHK024 Are architecture/security follow-ups routed to `FollowUpHardening` or existing evidence paths instead of new plan scope? [Consistency, Plan §Constitution Check]
...
- [ ] CHK025 Are all six local Spec-Kit presets named with the current versions in the plan? [Completeness, Plan §Summary]
...
- [ ] CHK026 Are NIST SSDF and CWE Top 25 retained as Level-2 context without creating artificial implementation tasks? [Governance, Plan §Constitution Check]
...
- [ ] CHK027 Are ASVS, SBOM, VEX, SLSA, OpenSSF Scorecard, AI-SBOM, NIS2, CRA, EU AI Act, and DORA `N/A` decisions trigger-based? [Governance, Contract §10]
...
- [ ] CHK028 Are STRIDE/CIA/CAPEC, S-ADR, arc42, Zero Trust, SAMM, BSI C3A, and BSI C5 `N/A` because architecture/cloud/deployment boundaries are unchanged? [Governance, Plan §Constitution Check]
...
- [ ] CHK029 Is cross-platform script governance explicitly `N/A` because no script-shaped tool changes? [Governance, Plan §Constitution Check]
...
- [ ] CHK030 Are DocFX and web-a11y validation triggers conditional and consistently described? [Consistency, Research Decision 6, Contract §8]
...
- [ ] CHK031 Are changed Markdown evidence and guidance required to remain text-first and accessible? [A11Y, Spec §FR-022]
...
- [ ] CHK032 Are agent guidance surfaces complete and synchronized when shared rules change? [Agent Parity, Spec §FR-023]
...
- [ ] CHK033 Is `.specify/templates/` impact explicitly `N/A` unless repository-owned templates are changed? [Agent Parity, Data Model §AgentGuidanceReview]
...
- [ ] CHK034 Are validation levels proportional to the touched artifact types? [Acceptance, Plan §Testing, Contract §11]
...
- [ ] CHK035 Is `dotnet format --verify-no-changes` present as a final implementation evidence requirement? [Completeness, Contract §11]
...
- [ ] CHK036 Are full Release tests and coverage gate required only when shared logic or broad proof helpers are materially touched? [Measurability, Quickstart §8]
...
- [ ] CHK037 Is branch versioning defined before any build, test, commit, or push on the numbered branch? [Consistency, Plan §Testing, Contract §11]
...
- [ ] CHK038 Can `/speckit-tasks` derive setup, evidence-ledger, hotspot-inventory, review, comment-edit, governance, validation, and statistics tasks without new decisions? [Readiness, Plan §Phase 2]
...
- [ ] CHK039 Are future task dependencies clear enough to avoid parallel edits to shared evidence or agent guidance? [Dependency, Plan §Phase 2]
...
- [ ] CHK040 Are completion signals explicit enough for a reviewer to accept the plan before implementation starts? [Acceptance, Plan §Post-Design Gate Review]
