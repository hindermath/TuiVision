...
- Optionales Projektprofil: `intake-review-governance` v0.2.3 prio 65 ist in TuiVision aktiv. Es bindet den naechsten Intake vor Feature-Erstellung an ein aktuelles `Ready`- oder menschlich akzeptiertes `ReadyWithAcceptedRisks`-Ergebnis; Review und Status bleiben read-only, Repair benoetigt ausdrueckliche Aenderungsautoritaet. *Optional project profile: `intake-review-governance` v0.2.3 at priority 65 is active in TuiVision and hash-binds the next intake before feature creation.*
...
- Die veröffentlichten eigenständigen GitHub-Preset-Repositories sind alleinige Produktquellen. Lokale Checkouts unter `~/SpecKitPresetProjects/` dienen ihrer Entwicklung; installierte Pakete sind Integrationskopien.
- Verbesserungen direkt im passenden eigenständigen Preset-Repository entwickeln, prüfen und veröffentlichen. Ein unveränderliches Release-Paket wird nur nach ausdrücklicher Freigabe hier integriert; keine lokalen Produkt-Forks oder Scaffold-Doppelpflege.
...
Kontext und Auftrag. Keine automatische GSDB-/C5-Prüfung oder Freigabe.
Produktquelle ist ausschließlich das veröffentlichte eigenständige GitHub-Repo.
