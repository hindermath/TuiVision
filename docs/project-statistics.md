...
| 2026-03-24 | Coverage-Gate-Synchronisierung fuer `006-close-phase8-gate` | Die harte Coverage-Regel wurde repo-weit auf `TuiVision.Core`, `TuiVision.Controls` und `TuiVision.Serialization` jeweils `>= 70 %` vereinheitlicht; dazu wurden `Pflichtenheft.md`, `docs/porting-status.md`, die Phase-8-Review-Checkliste sowie die gemeinsamen Agent-Dateien synchronisiert. |
| 2026-03-25 | `/speckit-plan` fuer `006-close-phase8-gate` | Planartefakte `plan.md`, `research.md`, `data-model.md`, `quickstart.md` und `contracts/phase-8-gate-contract.md` fuer den finalen `M-07`- und Phase-8-Gate-Abschluss angelegt; der Plan fixiert die 3x-70-%-Coverage-Huerde, den repository-weiten `dotnet test`-Lauf, die bedingte Linux/Windows/WSL-Evidenz und den dedizierten Gate-Closure-Commit als verbindlichen Arbeitsstand. |
